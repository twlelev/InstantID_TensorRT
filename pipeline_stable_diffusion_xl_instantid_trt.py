        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if 'controlnet' in self.args.stages:
                    cond_scale = torch.tensor([cond_scale], dtype=torch.float16, device=self.device_infer)
                    ...
                    params_controlnet.update({'controlnet_cond': image})
                ...
                if 'unet' in self.args.stages:
                    ...
                    noise_pred = self.runEngine('unet', params)['out_sample']  
                else:
